    ...
    <div class="relief-card status-{{ strtolower($event->status) }}">
        <div class="relief-card-header">
            ...
            <div class="relief-card-actions">
                ...
                <form action="{{ route('admin.relief.destroy', $event->id) }}" method="POST" 
        ...
        <div class="relief-card-footer">
            <div class="status-button-container">
                @if($event->status === 'Ongoing')
                    <button class="status-btn status-ongoing" onclick="event.preventDefault(); window.location.href='{{ route('admin.relief.show', $event->id) }}'">
                        <i class="fas fa-check-circle"></i>
                        <span>Mark as Finished</span>
                    </button>
                @endif
            </div>
            <a href="{{ route('admin.relief.show', $event->id) }}" class="view-details">View Details →</a>
        </div>
    </div>
</div>